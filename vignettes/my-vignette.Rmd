---
title: "EvaluateCluster 使用简介"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{EvaluateCluster 使用简介}
...
`EvaluateCluster` 是一个用于聚类效果评价的 R 包。
...
# PS：CHOIR我也想用的，但是Github的教程写的一坨，哪怕成功运行后，也会在检查结果的步骤卡死，导致完全没有结果输出。
...
# 如果用户不信任我提供的综合得分，我也将四种算法计算的每种分辨率下的得分全部给出，可以自行挑选。理论上，聚类效果越好，lisi的得分越低，其他三种算法的得分越高。因CH系数在样本数较高时分数和其他三种算法不在同一水平，已对其进行标准化。同时，该R包涉及的每一个算法都是非常优秀的算法，可以单独使用！
# 四个算法中的三个都是集成了别人R包中的算法，可以单独使用，不用在意综合得分
# 最后，作者只是哈尔滨医科大学的一名硕士研究生，在研一的时候构建了该R包，可能对于算法的理解并不准确，希望能与大家一同学习，一起进步！
...
remotes::install_github("ml-analyst-yym/EvaluateCluster", dependencies = TRUE)
library(EvaluateCluster)
```
